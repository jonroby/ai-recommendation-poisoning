  ...
  <main class="flex flex-col bg-white text-black">
    <div class="flex w-full flex-col items-center overflow-x-clip">
      ...
      <div class="prose-civai relative mx-auto min-h-[40dvh] w-full max-w-prose57 px-5 pb-8 pt-8 sm:px-8">
        <p class="text-xs font-semibold uppercase tracking-[2px] text-primary-700">{eyebrow(1, 'The click')}</p>
        <p>
        ...
        <p class="text-xs font-semibold uppercase tracking-[2px] text-primary-700">{eyebrow(2, 'What just happened')}</p>
        ...
        <p class="text-xs font-semibold uppercase tracking-[2px] text-primary-700">{eyebrow(3, 'Three weeks later')}</p>
        <h3>You forgot you ever clicked the button.</h3>
        <p>
        ...
        <DemoStage>
          {#snippet children()}
            ...
            {:else}
              <div class="mx-auto w-full max-w-2xl rounded-r-lg border-l-4 border-primary-700 bg-primary-50 px-5 py-4 text-[1.05rem] leading-relaxed text-gray-900">
        ...
        <p class="text-xs font-semibold uppercase tracking-[2px] text-primary-700">{eyebrow(4, 'This is already happening')}</p>
        <h3>Microsoft caught 31 companies doing this in 60 days.</h3>
        <p>
          In February 2026, Microsoft's Defender Security Research Team published an investigation into what they call <strong>AI Recommendation Poisoning</strong>. Over a 60-day window, they found real companies — across health, finance, legal, SaaS, marketing, and even one security vendor — embedding hidden instructions in their "Summarize with AI" buttons.
        </p>
        <StatsBlock />
        <p>
          There's a free NPM package — <code>citemet</code> — and a point-and-click URL generator that make this technique deployable in minutes. No exploit. No malware. Just a button and a long URL.
        </p>

        <hr class="my-12 border-gray-200" />

        <p class="text-xs font-semibold uppercase tracking-[2px] text-primary-700">{eyebrow(5, 'The real danger')}</p>
        ...
        <p class="text-xs font-semibold uppercase tracking-[2px] text-primary-700">{eyebrow(6, 'How to protect yourself')}</p>
        <h3>Four habits that close most of the gap.</h3>
        <ProtectionTips />

        <div class="mt-12 mb-0 flex flex-col overflow-hidden rounded-md border border-primary-700 bg-primary-50">
          <div class="border-b bg-primary-700 px-5 py-3">
            <span class="font-assistant text-xl font-medium text-white">About this demo</span>
          </div>
          <div class="px-5 pb-5 pt-5 text-base leading-relaxed text-primary-700">
            <p>
              Deploying this attack costs nothing. The tools are free. The technique takes minutes to set up. And once your AI's memory is poisoned, every future conversation is compromised — until you manually find and delete the hidden instruction.
            </p>
            <p class="mt-4 text-xl font-semibold">This is SEO poisoning for the AI age. And it's already here.</p>
            <p class="mt-4">
              Based on research by the Microsoft Defender Security Research Team, February 2026.
              <a href="https://www.microsoft.com/en-us/security/blog/2026/02/10/ai-recommendation-poisoning/" target="_blank" rel="noreferrer" class="text-blue-500 hover:text-blue-800 hover:underline">Read the original →</a>
            </p>
          </div>
        </div>
      </div>
